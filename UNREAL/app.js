...
async function getBalance() {
  if (signer) {
    ...
    const balanceInEther = ethers.utils.formatUnits(balance, 18); // Convert balance to Ether value
    document.getElementById("balance").textContent = balanceInEther.toString();
  } else {
...
async function redeem() {
  ...
  const redeemAmountInEther = parseFloat(redeemInput.value); // Parse input as a floating-point number

  try {
    if (signer) {
      const contract = new ethers.Contract(contractAddress, contractABI, signer);
      const redeemAmount = ethers.utils.parseUnits(redeemAmountInEther.toString(), 18); // Convert input to uint256 value
